...
namespace TriangleTests
{
    ...
    {
        private const string TRIANGLE_INFO_TEMPLATE = "Triangle: {0}({1}, {2}, {3}){0}({4:F2}, {5:F2}, {6:F2}){0}{0}Area = {7:F2}{0}Perimeter = {8}";
        ...
        public TriangleTest()
        {
            ITriangleService triangleService = new TriangleService();

            controller = new TriangleController(triangleService);
        }
        ...
        {
            string.Format("{0:F4}", area).Should().BeEquivalentTo(controller.Area(new Triangle(side1, side2, side3)).ToString());
        }
        ...
        {
            controller.Perimeter(new Triangle(side1, side2, side3)).Should().BeEquivalentTo(perimeter.ToString());
        }
